...
# Optional: render manga.yml description as Markdown (safe to keep even if unused)
try:
...
# Manifest generated by your GitHub Action
...
def parse_iso(dt: str | None) -> datetime:
    """Parse ISO time from manifest; return datetime.min on failure."""
    if not dt:
        return datetime.min
    try:
...
def load_updated_manifest() -> dict:
    """
    Return dict like: { manga_slug: { chapter_slug: ISO } }
    """
    try:
        ...
        # First run before the action creates the file
        return {}
    except Exception:
        return {}
...
def scan_content():
    ...
    for mdir in sorted(p for p in CONTENT_DIR.iterdir() if p.is_dir()):
        slug = mdir.name
        if slug.startswith("_"):  # skip internal folders
            continue
        ...
        # sort chapters ascending by number, tie-break by updated then dir time
...
def index():
    ...
    # Sort by last updated (manifest-powered)
...
def manga_detail(slug):
    ...
    return render_template("manga_detail.html",
        manga=manga, chapters=chapters_desc, naturaltime=naturaltime
    )
...
# --- Debug helper: see what the server thinks your "updated" times are ---
